...
internal sealed class EpisodePathParser
{
    ...
    public EpisodePathParserResult Parse(string path, bool isDirectory, bool? isNamed = null, bool? isOptimistic = null, bool? supportsAbsoluteNumbers = null, bool fillExtendedInfo = true)
    {
        ...
        {
            if (supportsAbsoluteNumbers.HasValue && expression.SupportsAbsoluteEpisodeNumbers != supportsAbsoluteNumbers.Value)
            {
                continue;
            }
    ...
    private static EpisodePathParserResult Parse(string name, EpisodeExpression expression)
    {
        ...
        if (match.Success && match.Groups.Count >= 3)
        {
            if (expression.IsByDate)
            {
                ...
                {
                    if (DateTimeOffset.TryParseExact(match.Groups[0].Value, expression.DateTimeFormats, CultureInfo.InvariantCulture, DateTimeStyles.None, out date))
                    {
                ...
                else if (DateTimeOffset.TryParse(match.Groups[0].Value, CultureInfo.InvariantCulture, DateTimeStyles.None, out date))
                {
                    result.Year = date.Year;
                    result.Month = date.Month;
                    result.Day = date.Day;
            ...
            else if (expression.IsNamed)
            {
                ...
                if (endingNumberGroup.Success)
                {
                    ...
                    if (nextIndex >= name.Length || "0123456789iIpP".IndexOf(name[nextIndex], StringComparison.Ordinal) == -1)
                    {
    ...
    private static void FillAdditional(string path, EpisodePathParserResult info, IEnumerable<EpisodeExpression> expressions)
    {
        ...
        foreach (var result in results)
        {
            ...
            if (!string.IsNullOrEmpty(info.SeriesName) && (!info.EpisodeNumber.HasValue || info.EndingEpisodeNumber.HasValue))
            {
                break;
            }
        }
